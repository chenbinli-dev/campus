  ...
  <div id="balance" class="balance">
    ...
    >
      <ul class="setInputList">
        <li><span class="title">设置支付密码，用于支付任务金</span></li>
        <li>
    ...
    >
      <ul class="updateInputList">
        <li><span class="title">修改支付密码</span></li>
        <li>
          <div class="firstInput">
            <span class="labelTitle">请输入支付密码</span>
            <password-input
...
<script>
import { NavBar, Icon, PullRefresh, Button as VanButton, Popup, Cell, PasswordInput, NumberKeyboard, Toast } from 'vant'
...
export default {
  ...
  methods: {
    ...
    getUserBalanceAndPayPasswordByUid () {
      userRequest.get('/user/getUserBalanceAndPayPasswordByUid', {
    ...
    //提交支付密码
    Confirm () {
      if (this.firstInput.length < 6 || this.secondInput.length < 6 || !this.firstInput || !this.secondInput) {
        Toast({
          type: 'fail',
          message: '信息不完整'
        })
        ...
        userRequest.post('/balance/createPayPassword', this.confirm, {
    ...
    //更新用户支付密码
    Update () {
      if (this.oldPasswordInput.length < 6 || this.newPasswordInput.length < 6 || !this.oldPasswordInput || !this.newPasswordInput) {
        Toast({
          type: 'fail',
          message: '信息不完整'
        })
        ...
        userRequest.post('/balance/updatePayPassword', this.update, {
          headers: { 'Authorization': localStorage.getItem('TOKEN') }
        })
          .then(res => {
