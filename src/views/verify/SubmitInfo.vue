<template>
  <div id="submitInfo" class="submitInfo">
    ...
    <!--姓名输入-->
    <van-dialog title="姓名" v-model="nameShow" show-cancel-button @confirm="nameConfirm">
      <field v-model="name" autofocus="true" />
    </van-dialog>
    <!--身份证号输入-->
    <van-dialog title="身份证号" v-model="idNumberShow" show-cancel-button @confirm="idNumerConfirm">
      <field v-model="id_number" autofocus="true" />
    </van-dialog>
    <!--省份选择-->
    <popup v-model="provicneShow" round position="bottom" :style="{ height: '30%' }">
      <picker
        title="学校所在省份"
        show-toolbar
        :columns="provinces"
    ...
    <!--入学年份选择-->
    <popup v-model="enrollmentYearShow" round position="bottom" :style="{ height: '30%' }">
      <picker
        title="入学年份"
        show-toolbar
        :columns="years"
  ...
  methods: {
    ...
    submit() {
      ...
      if (
        ...
        (this.studentInfo.id_number && this.studentInfo.id_number.length < 16) ||
        ...
        //信息校验通过，提交后台
        userRequest
          .post('/user/submitStudentInfo', this.studentInfo, {
